...
const effectiveCardData = computed(() => props.cards ?? cardData);
...
const baseClassName = computed(() => `card ${props.enableBorderGlow ? 'card--border-glow' : ''}`);
...
<template>
  ...
  >
    <div class="card-responsive">
      <template v-for="(card, index) in effectiveCardData" :key="index">
        <ParticleCard
          v-if="enableStars"
          :class="baseClassName"
          :style="{ ...getCardStyle(card), cursor: card.to ? 'pointer' : 'default' }"
          :disable-animations="shouldDisableAnimations"
        ...
        >
          ...
          <div class="card__content">
            <h3 :class="`card__title ${textAutoHide ? 'text-clamp-1' : ''}`">
              {{ card.title }}
            </h3>
            <p :class="`card__description ${textAutoHide ? 'text-clamp-2' : ''}`">
              {{ card.description }}
            </p>
          </div>
        </ParticleCard>
